...
app.post('/', function(req, res) {
    ...
    if (data.object === 'page') {
      ...
      User.findOne({ 'organization': req.body.organization }, (err, user) => {
        if (err) {
          console.log(err)
        }
        ...
        var getSendees = new Promise(function(resolve, reject) {
          for (var i = 0; i < req.body.groupNames.length; i++) {
            Group.findOne({ groupName: req.body.groupNames[i], organization: req.body.organization }, (err, group) => {
              if (err) {
        ...
        getSendees.then((sendees) => {
          for (var i = 0; i < sendees.length; i++) {
            switch (req.body.type) {
              ...
              console.log('msg text: ' + req.body.assetManifest.text)
              sendTextMessage(sendees[i], user.facebook.accessToken, req.body.assetManifest.text)
              break
              case 'image':
